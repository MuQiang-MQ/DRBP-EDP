...
def load_models(model_checkpoint):
    try:
        ...
        model_path_stage1 = os.path.join(application_path, 'best_model_stage1.pth')
        if not os.path.exists(model_path_stage1):
            st.error(f"Model file not found: {model_path_stage1}")
            return None, None, False
        state_dict_stage1 = torch.load(model_path_stage1, map_location=device)
        ...
        model_path_stage2 = os.path.join(application_path, 'best_model_stage2.pth')
        if not os.path.exists(model_path_stage2):
            st.error(f"Model file not found: {model_path_stage2}")
            return None, None, False
        state_dict_stage2 = torch.load(model_path_stage2, map_location=device)
...
def main():
    ...
    # Load Tokenizer
    model_checkpoint = application_path  # 当前目录
